...
class CUR(PyMFBase3):
    ...
    def computeUCR(self):
        ...
        if scipy.sparse.issparse(self.data):
            ...
            self._U = np.linalg.pinv(self._C, self._k) * self.data[:,:] * np.linalg.pinv(self._R, self._k)
            ...
            self._U = np.dot(np.dot(np.linalg.pinv(self._C, self._k), self.data[:,:]),
                             np.linalg.pinv(self._R, self._k))
